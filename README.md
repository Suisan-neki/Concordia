...
詳細は [`docs/VALUE_PROPOSITION.md`](docs/VALUE_PROPOSITION.md) を参照。
...
### 2.5 基本姿勢：合意をめぐる複雑性と技術の役割

#### 合意は誰のものか
医療における合意は、患者本人、家族、病院方針、文化的規範などが絡み合う複雑なものである。これら「見えない影響力」を排除するのではなく、むしろ**人間の意思決定に本質的に内在するもの**として受け入れる。

Concordia が技術で担保すべきは、「圧力の排除」ではなく以下である：

- **自発性の確保**: 本人の声が自分の言葉で残る仕組み
- **責任の所在**: 誰の理由でどの方向に傾いたかが後から辿れる
- **可逆性の担保**: 重要度と不可逆度に応じた選択肢の確保
- **語りの構造**: 結論だけでなく、理解しようとした軌跡が残る

#### 段階的な複雑化
すべての場面で同じガードレールが要るわけではない。

- **一意に定まる場面**: 簡潔な確認で十分。過剰な手続きは「使うのが面倒」になり本末転倒
- **複雑性が高い場面**: 家族関係・病院方針・文化的圧力が絡む時は、第三者立会い・クールオフ・再確認プロセスを強化
- **緊急時の例外**: 即時のケアを優先しつつ、事後の丁寧な説明とレビューを必須化

#### 医療を超えた汎用性
Concordia の設計原理は医療だけに限定されない。**対話の透明性と安心**を技術で支えるという思想は、説明責任が問われるあらゆる場面に応用可能である。
