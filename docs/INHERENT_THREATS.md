...
本ドキュメントでは、医師/患者の非対称を中心に扱ってきたが、実運用では組織文化・病院方針・家族関係が理解行動に与える圧力が大きい。

- 病院方針/文化の影響
  - 圧力源: 説明時間の短縮圧、監査優先の発話抑制。
  - 技術応答: 目的限定可視性（監査=合意の有無のみ）、封（Capsule）で論点を切り出して共有、数値比較を避ける。

- 家族の影響
  - 圧力源: 代理意思決定の同調圧、発言の萎縮。
  - 技術応答: 可視性制御（家族=メタデータのみ）、匿名/遅延Clarify、合意しない権利の明示。

- 方針
  - 設計の態度: 技術は圧力を増やし得る。常に“責めない”表示と最小開示を優先する。
  - 運用連携: 倫理/リスク委員会のガイドラインと整合し、技術は「検証可能性」を提供するに留める。
